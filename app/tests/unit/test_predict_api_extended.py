...
# -----------------------------------------------------------------------
cold_msgs = [
    "just browsing", "maybe later", "not interested", "what is this", "send info",
    "hmm", "ok", "thanks", "hello", "hi", "looking around", "not sure",
    "don't know", "whatever", "k", "cool", "seen it", "bye", "no thanks", "later"
]
cold_cases = []
for i in range(20):
...
# -----------------------------------------------------------------------
warm_keywords = ["demo", "pricing", "how", "compare", "features", "budget", "team", "options", "tell me", "more info"]
warm_templates = [
    "Can I see a {kw}?", "How much is {kw}?", "What are your {kw}?", "How does it {kw}?",
    "I'm comparing {kw}", "Do you have {kw}?", "Tell me about {kw}", "What’s the {kw}?"
]
for i in range(25):
    kw = random.choice(warm_keywords)
    msg = random.choice(warm_templates).format(kw=kw)
    if i >= 20:
        msg = rand_msg(warm_keywords + ["for my team", "for business", "next quarter"], [kw])
    TEST_CASES.append({
...
# -----------------------------------------------------------------------
hot_keywords = ["buy", "purchase", "start", "today", "ASAP", "now", "implement", "sign", "contract", "close"]
hot_templates = [
    "I want to {kw} now", "Can we {kw} today?", "Ready to {kw}", "Let's {kw} ASAP",
    "Need this {kw} immediately", "When can we {kw}?", "I'm ready to {kw}"
]
for i in range(20):
    kw = random.choice(hot_keywords)
    msg = random.choice(hot_templates).format(kw=kw)
    if i >= 15:
        msg += " " + random.choice(["Let’s do it!", "Send contract", "Credit card ready"])
    TEST_CASES.append({
...
# -----------------------------------------------------------------------
return_templates = [
    "Time to renew", "Want to upgrade", "Need new quote", "Add features",
    "Loved it last time", "Back for more", "Extend contract", "Bigger plan"
]
for i in range(15):
